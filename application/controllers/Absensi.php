...
class Absensi extends CI_Controller
{
    ...
    {
        return $this->template->load('template', 'absensi/absen_masuk');
    }
    ...
    public function absen()
    {
        ...
        $jam_masuk = (rand(7, 8));
        if ($jam_masuk === 8) {
            $detik_masuk = (rand(0, 15));
            $micro_detik_masuk = (rand(0, 59));
        } else {
            $detik_masuk = (rand(0, 59));
            $micro_detik_masuk = (rand(0, 59));
        }
        $jadi_masuk = $jam_masuk . ":" . $detik_masuk . ":" . $micro_detik_masuk;
        $waktu_masuk = strtotime($jadi_masuk);


        $jam_pulang = (rand(16, 18));
        $detik_pulang = (rand(0, 59));
        $micro_detik_pulang = (rand(0, 59));
        $jadi_pulang = $jam_pulang . ":" . $detik_pulang . ":" . $micro_detik_pulang;
        $waktu_pulang = strtotime($jadi_pulang);


        $data_masuk = [
            'tgl' => $post['tgl'],
            'waktu' => date('H:i:s', $waktu_masuk),
            'keterangan' => 'Masuk',
            'id_user' => $this->session->id_user
        ];
        $result = $this->absensi->insert_data($data_masuk);

        $data_pulang = [
            'tgl' => $post['tgl'],
            'waktu' => date('H:i:s', $waktu_pulang),
            'keterangan' => 'Pulang',
            'id_user' => $this->session->id_user
        ];
        $result = $this->absensi->insert_data($data_pulang);

        $data = [
            'tgl' => $post['tgl'],
            'waktu_pulang' =>  date('H:i:s', $waktu_pulang),
